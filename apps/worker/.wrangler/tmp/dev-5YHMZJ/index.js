...
var require_matcher = __commonJS({
  "node_modules/node-html-parser/dist/matcher.js"(exports) {
    ...
    function findOne(test, elems) {
      let elem = null;
      for (let i2 = 0, l2 = elems === null || elems === void 0 ? void 0 : elems.length; i2 < l2 && !elem; i2++) {
        const el = elems[i2];
        if (test(el)) {
          elem = el;
        } else {
  ...
  "node_modules/node-html-parser/dist/nodes/html.js"(exports) {
    ...
    var HTMLElement = class _HTMLElement extends node_1.default {
      ...
      closest(selector) {
        ...
        function findOne(test, elems) {
          let elem = null;
          for (let i2 = 0, l2 = elems.length; i2 < l2 && !elem; i2++) {
            const el2 = elems[i2];
            if (test(el2)) {
              elem = el2;
            } else {
      ...
      insertAdjacentHTML(where, html) {
        ...
        const p2 = parse2(html, this._parseOptions);
        if (where === "afterend") {
          this.after(...p2.childNodes);
        } else if (where === "afterbegin") {
          this.prepend(...p2.childNodes);
        } else if (where === "beforeend") {
          this.append(...p2.childNodes);
        } else if (where === "beforebegin") {
          this.before(...p2.childNodes);
        } else {
...
// .wrangler/tmp/bundle-tvDrlW/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-tvDrlW/middleware-insertion-facade.js
init_modules_watch_stub();
...
__name(slug, "slug");
function parseRelative(s2) {
  const t = s2.toLowerCase().replace(/[()]/g, "").replace(/\bago\b/g, "").trim();
  const m = t.match(
...
__name(parseRelative, "parseRelative");
function relToIsoBucketed(s2, bucketCounts, stepMs = 6e4) {
  const { ms, bucket } = parseRelative(s2);
  const idx = bucketCounts.get(bucket) ?? 0;
  bucketCounts.set(bucket, idx + 1);
  return new Date(Date.now() - ms - idx * stepMs).toISOString();
}
__name(relToIsoBucketed, "relToIsoBucketed");
function buildWarmapFromHtml(html, attackWindowMin = 7) {
  const doc = (0, import_node_html_parser.parse)(html);
  const keeps = [];
  const keepDivs = doc.querySelectorAll("div.keepinfo");
  for (const div of keepDivs) {
    const name = div.querySelector("strong")?.text.trim() || div.getAttribute("id")?.replace(/_/g, " ") || "Unknown";
    const classes = (div.getAttribute("class") || "").toLowerCase();
    const realmKey = classes.match(/keepinfo_(alb|mid|hib|albion|midgard|hibernia)/)?.[1] || "alb";
    ...
    const emblemSrc = div.querySelector('img[alt*="emblem" i]')?.getAttribute("src") ?? div.querySelector('img[src*="emblem"]')?.getAttribute("src") ?? null;
    const emblem = emblemSrc ? new URL(emblemSrc, "https://herald.uthgard.net/").toString() : null;
    const headerCell = div.querySelector('td[align="center"]') ?? div;
    let claimedBy = null;
    if (headerCell) {
      const lines = headerCell.innerText.split("\n").map((s2) => s2.trim()).filter(Boolean);
      while (lines.length) {
        const last = lines[lines.length - 1];
        if (last === name || /^\(.*level/i.test(last) || /level/i.test(last) || /emblem/i.test(last)) {
          lines.pop();
          continue;
        }
        claimedBy = last;
  ...
  const windowMs = attackWindowMin * 6e4;
  const now = Date.now();
  for (const e2 of events) {
    if (e2.kind !== "underAttack") continue;
    const k = byId.get(e2.keepId);
    if (!k) continue;
    const t = Date.parse(e2.at);
...
__name(buildWarmapFromHtml, "buildWarmapFromHtml");
var router = e();
router.get("/api/warmap.json", async (_request, environment) => {
  const data = await environment.WARMAP.get("warmap", "json");
...
router.post("/admin/kv-test", async (_request, environment) => {
...
router.post("/admin/update", async (_request, environment) => {
  try {
    const res = await fetch(environment.HERALD_WARMAP_URL, {
      headers: {
    ...
    const payload = buildWarmapFromHtml(
      html,
      Number(environment.ATTACK_WINDOW_MIN ?? "7")
    );
...
var src_default = {
  fetch: /* @__PURE__ */ __name((request, environment, context) => router.handle(request, environment, context).catch((err) => {
    console.error("Unhandled error:", err);
    return new Response("Internal Server Error", { status: 500 });
  }), "fetch"),
  scheduled: /* @__PURE__ */ __name(async (_event, environment) => {
    const payload = {
      updatedAt: (/* @__PURE__ */ new Date()).toISOString(),
      keeps: [],
      events: []
